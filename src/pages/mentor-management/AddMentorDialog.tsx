...
export const AddMentorDialog: React.FC<{ onCreated?: () => void }> = ({ onCreated }) => {
  ...
  const [form, setForm] = useState<CreateMentorData & { status: 'active' | 'inactive' | 'on_leave' }>({
    email: '',
    first_name: '',
    last_name: '',
    phone: '',
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('open-add-mentor-dialog', handler as EventListener);
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    const res = await MentorsService.createMentor(mentorData, profile?.user_id || '');
    if (res.success) {
