...
export const SemesterCard: React.FC<SemesterCardProps> = ({
  ...
  const getSemesterTitle = () => {
    return selectedPaymentPlan === 'one_shot' ? 'Program Fee' : `Semester ${semester.semesterNumber}`;
  };

  const getPaymentTypeBadge = () => {
    switch (selectedPaymentPlan) {
      case 'one_shot': return 'Full Payment';
      case 'sem_wise': return 'Semester Payment';
      case 'instalment_wise': return `${semester.instalments.length} Installments`;
  ...
  const getPaymentTypeDescription = () => {
    switch (selectedPaymentPlan) {
      case 'one_shot': return 'One-time payment';
      case 'sem_wise': return 'Due at semester start';
      case 'instalment_wise': return 'Monthly installments';
      default: return 'Payment';
    }
  };
    ...
    <Card className="overflow-hidden">
      ...
      <CardContent>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-4">
          <div>
            ...
            <p className="text-lg font-semibold">{formatCurrency(semester.total.baseAmount)}</p>
          </div>
          {semester.total.discountAmount > 0 && (
            <div>
              <p className="text-sm text-muted-foreground">Discount</p>
              <p className="text-lg font-semibold text-green-600">-{formatCurrency(semester.total.discountAmount)}</p>
            </div>
          )}
          <div>
            <p className="text-sm text-muted-foreground">GST</p>
            <p className="text-lg font-semibold">{formatCurrency(semester.total.gstAmount)}</p>
          </div>
          <div>
      ...
      {isExpanded && (
        <div className="space-y-4 mt-4">
          {semester.instalments.map((installment: SemesterInstalment, index: number) => (
            <InstallmentItem
              key={index}
