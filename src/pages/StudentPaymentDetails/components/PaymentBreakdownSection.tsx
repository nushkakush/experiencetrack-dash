            ...
            <CardContent className="pt-0">
              ...
              <div className="space-y-4">
                {semester.installments?.map((installment: Instalment, index: number) => {
                  ...
                  return (
                    <div key={installmentKey} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                          <DollarSign className="h-4 w-4" />
                          <span className="font-medium">
                            Installment {index + 1} - {formatCurrency(installment.amount)}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => onToggleInstallment(installmentKey)}
                          >
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <p className="font-medium">{formatCurrency(installment.paidAmount || 0)}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Pending</p>
                          <p className="font-medium">{formatCurrency(installment.pendingAmount || 0)}</p>
                        </div>
                      </div>
                      ...
                      {expandedInstallments.has(installmentKey) && (
                        <div className="mt-4 pt-4 border-t">
                          <p className="text-sm text-muted-foreground mb-2">
                            Payment details and submission form would go here
                          </p>
                        </div>
                      )}
                    </div>
