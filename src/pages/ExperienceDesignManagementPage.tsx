...
import { Plus, Search, Filter, Eye, Edit, Trash2, Calendar, User, FileText, Zap, Sparkles } from 'lucide-react';
...
const ExperienceDesignManagementPage: React.FC = () => {
  ...
  const handleCreateDialogClose = (open: boolean) => {
    console.log('📝 handleCreateDialogClose called with:', { open, currentState: createDialogOpen });
    setCreateDialogOpen(open);
  };

  const handleEditDialogClose = (open: boolean) => {
    console.log('✏️ handleEditDialogClose called with:', { open, currentState: editDialogOpen });
    ...
    <DashboardShell>
      <div className='space-y-6'>
        ...
        <div className='flex items-center justify-between'>
          <div>
            <h1 className='text-3xl font-bold tracking-tight'>Experience Design</h1>
            <p className='text-muted-foreground'>
              Design and manage learning experiences with structured content and assessment.
            </p>
          </div>
        ...
        <Tabs defaultValue="experiences" className="space-y-6">
          <TabsList>
            ...
            <TabsTrigger value="magic-briefs" className="flex items-center gap-2">
      ...
      {!activeEpicId && (
        <div className='border-2 border-dashed border-muted rounded-lg p-8 text-center'>
          ...
          <h3 className='text-lg font-medium mb-2'>Select an Epic to Get Started</h3>
          <p className='text-muted-foreground mb-4'>
            Choose an epic from the dropdown above to view and manage experiences for that epic.
          </p>
        </div>
      )}
                  ...
                  <TableCell>
                    <div className='max-w-xs'>
                      {experience.learning_outcomes && experience.learning_outcomes.length > 0 ? (
                        <div className='text-sm text-muted-foreground'>
                          {experience.learning_outcomes.length} outcome{experience.learning_outcomes.length !== 1 ? 's' : ''}
                        </div>
                      ) : (
                        <div className='text-sm text-muted-foreground'>No outcomes defined</div>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
      ...
      {activeEpicId && totalPages > 1 && (
        <div className='flex items-center justify-between'>
          <div className='text-sm text-muted-foreground'>
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, totalCount)} of {totalCount} experiences
          </div>
