...
export function buildPromptInstructions(
  ...
  const outcomeInstruction = remainingOutcomes.length > 0 
...
export function buildMagicBriefPrompt(
  ...
  const instructions = buildPromptInstructions(request, startupStage, remainingOutcomes);
  ...
  const template = PROMPT_TEMPLATES.GENERATION_BASE
    .replace('{coreRequirements}', coreRequirements)
    .replace('{epic_name}', request.epic_name || 'Unknown Epic')
    .replace('{epic_description}', request.epic_description || 'No description available')
    .replace('{epic_outcomes}', request.epic_outcomes.slice(0, 5).join(', ') + (request.epic_outcomes.length > 5 ? ` (and ${request.epic_outcomes.length - 5} more outcomes)` : ''))
    .replace('{brief_index}', request.brief_index?.toString() || '1')
    .replace('{total_briefs}', request.total_briefs?.toString() || '7')
    .replace('{covered_outcomes}', request.previous_briefs?.flatMap(b => b.connected_learning_outcomes).join(', ') || 'None')
...
export function buildMagicBriefExpansionPrompt(): string {
  const coreRequirements = [
    ...
    CORE_REQUIREMENTS.CITATION_REQUIREMENTS,
    CORE_REQUIREMENTS.RESEARCH_METHODOLOGY
  ].join('\n\n');

  const template = PROMPT_TEMPLATES.EXPANSION_BASE
    .replace('{coreRequirements}', coreRequirements)
    .replace('{jsonResponse}', CORE_REQUIREMENTS.JSON_RESPONSE)
