  ...
  switch (status) {
    ...
    case 'on_time':
      return {
        ...
        text: status === 'paid' ? 'Paid' : status === 'complete' ? 'Complete' : 'On-Time'
      };
    ...
    case 'not_setup':
      return {
        ...
        text: status === 'pending' ? 'Pending' : status === 'upcoming' ? 'Upcoming' : 'Not Setup'
      };
    ...
    case 'partially_paid_verification_pending':
      return {
        ...
        text: status === 'verification_pending' ? 'Verification Pending' : 'Partially Paid Verification Pending'
      };
    ...
    case 'awaiting_bank_approval_physical_mandate':
      return {
        ...
        text: status === 'awaiting_bank_approval_e_nach' ? 'Awaiting Bank Approval E-NACH' : 'Awaiting Bank Approval Physical Mandate'
      };
    ...
    case 'setup_request_failed_physical_mandate':
      return {
        variant: 'destructive' as const,
        className: 'bg-red-100 text-red-800 border-red-200 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800',
        text: status === 'setup_request_failed_e_nach' ? 'Setup Request Failed E-NACH' : 'Setup Request Failed Physical Mandate'
      };
