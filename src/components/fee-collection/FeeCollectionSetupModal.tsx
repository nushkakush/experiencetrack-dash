...
export const FeeCollectionSetupModal: React.FC<
  ...
  const memoizedFeeStructure = useMemo(() => {
    if (existingFeeStructure) {
      return {
        ...existingFeeStructure,
        admission_fee:
          feeStructureData.admission_fee || existingFeeStructure.admission_fee,
        total_program_fee:
          feeStructureData.total_program_fee ||
          existingFeeStructure.total_program_fee,
        number_of_semesters:
          feeStructureData.number_of_semesters ||
          existingFeeStructure.number_of_semesters,
        instalments_per_semester:
          feeStructureData.instalments_per_semester ||
          existingFeeStructure.instalments_per_semester,
        one_shot_discount_percentage:
          feeStructureData.one_shot_discount_percentage ||
          existingFeeStructure.one_shot_discount_percentage,
    ...
    return {
      ...
      one_shot_discount_percentage:
        feeStructureData.one_shot_discount_percentage || 0,
      program_fee_includes_gst: feeStructureData.program_fee_includes_gst,
      equal_scholarship_distribution:
        feeStructureData.equal_scholarship_distribution,
  ...
  const handleSaveClick = async () => {
    if (variant === 'student-custom') {
      ...
      try {
        ...
        const datesForSelected =
          (editedDates as Record<string, unknown>)[plan] || {};
      ...
      case 2:
        if (hideScholarships) {
          return (
            <div className='text-sm text-muted-foreground'>
              Scholarships are managed at the cohort level and are not editable
              in this flow.
            </div>
          );
        }
        return (
          <Step2Scholarships
            ...
            errors={{}}
            isReadOnly={
              variant === 'student-custom' || effectiveMode === 'view'
            }
          />
        );
      case 3:
        return (
          <Step3Review
            ...
            selectedPaymentPlan={selectedPaymentPlan}
            restrictToPlan={
              restrictPaymentPlanToSelected ? selectedPaymentPlan : undefined
            }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='max-w-6xl max-h-[90vh] flex flex-col'>
        <DialogHeader>
          ...
          <DialogDescription>
            Configure your cohort's fee structure, define scholarships, and
            preview the resulting payment plans.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className='px-6 py-4 border-t bg-background'>
          <StepNavigation
            steps={[
              ...
              ...(hideScholarships ? [] : [{ id: 2, title: 'Scholarships' }]),
              { id: 3, title: 'Review' },
            ]}
            ...
            onEdit={
              effectiveMode === 'view' && canEditFeeStructure
                ? () => onModeChange?.('edit')
                : undefined
            }
            onCancelEdit={
            ...
            saveButtonText={
              variant === 'student-custom' ? 'Save Custom Plan' : undefined
            }
          />
        </div>
      </DialogContent>
