    ...
    <Card key={semester.semesterNumber}>
      <CardHeader>
        <CardTitle className='text-blue-600'>
          Semester {String(semester.semesterNumber).padStart(2, '0')}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <Table>
          ...
          <TableBody>
            {semester.instalments.map(
              (instalment: SemesterInstalment, index: number) => {
                ...
                return (
                  <TableRow key={index}>
                    <TableCell>
                      {isReadOnly ? (
                        ...
                        <Input
                          ...
                          value={currentDate}
                          onChange={e =>
                            onPaymentDateChange(dateKey, e.target.value)
                          }
                          className='w-40'
                        />
                      )}
                    ...
                    <TableCell>
                      {formatCurrency(instalment.baseAmount)}
                    </TableCell>
                    <TableCell className='text-red-600'>
                      {instalment.scholarshipAmount > 0
                        ? `- ${formatCurrency(instalment.scholarshipAmount)}`
                        : '--'}
                    </TableCell>
                    <TableCell>
                      {formatCurrency(instalment.gstAmount)}
                    </TableCell>
                    <TableCell>
                      {formatCurrency(instalment.amountPayable)}
                    </TableCell>
                  </TableRow>
                );
              }
          ...
          <div className='space-y-2 text-sm'>
            ...
            {semester.total.scholarshipAmount > 0 && (
              <div className='flex justify-between text-red-600'>
                ...
                <span>
                  - {formatCurrency(semester.total.scholarshipAmount)}
                </span>
              </div>
            )}
