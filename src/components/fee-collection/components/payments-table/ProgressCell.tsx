...
export const ProgressCell: React.FC<ProgressCellProps> = React.memo(
  ({ student, feeStructure }) => {
    const [progressData, setProgressData] =
      useState<ProgressCalculationResult | null>(null);
    const [loading, setLoading] = useState(true);

    // Memoize the calculation to avoid unnecessary re-calculations
    const calculationKey = useMemo(() => {
      return `${student.student_id}-${student.paid_amount}-${student.total_amount}-${student.payment_plan}`;
    }, [
      student.student_id,
      student.paid_amount,
    ...
    useEffect(() => {
      const calculateProgress = async () => {
        try {
          setLoading(true);
          const result = await ProgressCalculator.getProgress(
            student,
            feeStructure
          );
      ...
      <TableCell>
        <div className='flex flex-col gap-2 min-w-[140px]'>
          ...
          <div className='text-xs text-muted-foreground'>
            {formatCurrency(progressData.paidAmount)} /{' '}
            {formatCurrency(progressData.totalAmount)}
          </div>
        </div>
      </TableCell>
    );
