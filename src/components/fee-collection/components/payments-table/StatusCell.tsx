...
export const StatusCell: React.FC<StatusCellProps> = ({
  ...
  const calculateExpectedAmount = async (): Promise<number> => {
    try {
      ...
      if (
        ...
        try {
          ...
          if (student.payment_plan === 'one_shot' && breakdown.oneShotPayment) {
            ...
            console.log(
              '🔍 [StatusCell] Semester-wise payment amount:',
              amount
            );
            return amount;
          } else if (
            ...
            if (firstInstallment) {
              ...
              console.log(
                '🔍 [StatusCell] Installment-wise payment amount:',
                amount
              );
              return amount;
            }
      ...
      if (feeStructure && student.payment_plan) {
        ...
        console.log(
          '🔍 [StatusCell] Fallback calculation amount:',
          calculatedAmount
        );
      ...
      if (student.payments && student.payments.length > 0) {
        ...
        if (paymentAmounts.length > 0) {
          ...
          console.log(
            '🔍 [StatusCell] Using largest payment amount as fallback:',
            largestPayment
          );
      ...
      console.warn(
        '🔍 [StatusCell] Could not calculate expected amount, returning 0'
      );
      return 0;
    } catch (error) {
    ...
    const fetchPartialPaymentInfo = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          console.log(
            '🔍 [StatusCell] Retrieved transactions:',
            transactions.map(t => ({
              id: t.id,
              amount: t.amount,
          ...
          const partialTransactions = transactions.filter(
            t => t.partial_payment_sequence && t.partial_payment_sequence > 0
          );

          console.log(
            '🔍 [StatusCell] Transactions with partial sequence found:',
            partialTransactions.length
          );

          // Calculate expected amount
          const expectedAmount = await calculateExpectedAmount();
          console.log(
            '🔍 [StatusCell] Calculated expected amount:',
            expectedAmount
          );

          if (partialTransactions.length > 0) {
            // Find the latest partial payment sequence
            const latestPartial = partialTransactions.reduce(
              (latest, current) =>
                current.partial_payment_sequence! >
                latest.partial_payment_sequence!
                  ? current
                  : latest
            );

            // Calculate total amounts
            const totalAmount = partialTransactions.reduce(
  ...
  const getOverallStatus = (): { status: PaymentStatus; text: string } => {
    ...
    if (!student.payment_plan || student.payment_plan === 'not_selected') {
      console.log(
        '🔍 [StatusCell] No payment plan selected, returning pending'
      );
    ...
    console.log(
      '🔍 [StatusCell] Using fallback logic with payments:',
      actualPayments
    );
    ...
    if (hasVerificationPending) {
      console.log(
        '🔍 [StatusCell] Has verification pending, returning verification_pending'
      );
      return {
    ...
    const paidPayments = actualPayments.filter(
    ...
    if (paidPayments.length > 0) {
      console.log(
        '🔍 [StatusCell] Some payments are paid, returning partially_paid'
      );
  ...
  const getOverallStatusDisplay = (
    status: string
  ): { status: PaymentStatus; text: string } => {
    console.log(
      '🔍 [StatusCell] getOverallStatusDisplay called with status:',
      status
    );
    ...
    switch (status) {
      ...
      case 'complete':
        console.log('🔍 [StatusCell] Returning complete status');
      ...
      case 'partially_paid_verification_pending':
        console.log(
          '🔍 [StatusCell] Returning partially_paid_verification_pending status'
        );
        return {
          status: 'partially_paid_verification_pending',
          text: 'Partial Payment Under Review',
        };
      case 'partially_paid_days_left':
        console.log(
          '🔍 [StatusCell] Returning partially_paid_days_left status'
        );
      ...
      default:
        console.log(
          '🔍 [StatusCell] Returning default pending status for unknown status:',
          status
        );
        return { status: 'pending', text: 'Payments Pending' };
    }
  ...
  const isActuallyPartialPayment = () => {
    ...
    if (partialPaymentInfo.expectedAmount > 0) {
      ...
      if (partialPaymentInfo.totalAmount >= partialPaymentInfo.expectedAmount) {
        console.log(
          '🔍 [StatusCell] Payment is complete (total >= expected):',
          {
            totalAmount: partialPaymentInfo.totalAmount,
            expectedAmount: partialPaymentInfo.expectedAmount,
            isComplete:
              partialPaymentInfo.totalAmount >=
              partialPaymentInfo.expectedAmount,
          }
        );
        return false;
      }
      ...
      if (partialPaymentInfo.paidAmount >= partialPaymentInfo.expectedAmount) {
        console.log('🔍 [StatusCell] Payment is complete (paid >= expected):', {
          ...
          expectedAmount: partialPaymentInfo.expectedAmount,
          isComplete:
            partialPaymentInfo.paidAmount >= partialPaymentInfo.expectedAmount,
        });
        return false;
      }
    } else {
      ...
      const isRoundAmount =
        partialPaymentInfo.totalAmount % 1000 === 0 ||
        partialPaymentInfo.totalAmount % 500 === 0;
      const isSingleTransaction =
        partialPaymentInfo.totalAmount === partialPaymentInfo.paidAmount ||
        partialPaymentInfo.paidAmount === 0;

      if (
        isRoundAmount &&
        isSingleTransaction &&
        partialPaymentInfo.totalAmount > 0
      ) {
        console.log(
          '🔍 [StatusCell] Payment appears to be complete (round amount, single transaction):',
          {
            totalAmount: partialPaymentInfo.totalAmount,
            isRoundAmount,
            isSingleTransaction,
    ...
    if (partialPaymentInfo.paidAmount >= partialPaymentInfo.totalAmount) {
      console.log('🔍 [StatusCell] Payment is complete (paid >= total):', {
        ...
        totalAmount: partialPaymentInfo.totalAmount,
        isComplete:
          partialPaymentInfo.paidAmount >= partialPaymentInfo.totalAmount,
      });
      return false;
    }
  ...
  return (
    <TableCell>
      <div className='space-y-1'>
        ...
        {partialPaymentInfo.hasPartialPayments &&
          isActuallyPartialPayment() && (
