...
export const PaymentAmountInput = React.memo<PaymentAmountInputProps>(
  ({
    ...
    const [loadingPartialConfig, setLoadingPartialConfig] =
    ...
    React.useEffect(() => {
      const fetchPartialPaymentConfig = async () => {
        if (!selectedInstallment || !studentId) {
          console.log(
            '🔒 [PaymentAmountInput] Skipping partial payment config fetch:',
            {
        ...
        try {
          ...
          console.log(
            '🔒 [PaymentAmountInput] Fetching partial payment config for:',
            {
              studentId,
          ...
          const config =
            await PartialPaymentsService.getInstallmentPartialPaymentConfig(
              studentId,
          ...
          console.log(
            '🔒 [PaymentAmountInput] Retrieved partial payment config:',
            config
          );
          setPartialPaymentConfig(config);
        } catch (error) {
          console.error(
            '🚨 [PaymentAmountInput] Error fetching partial payment config:',
            error
          );
    ...
    React.useEffect(() => {
      const allowPartialPayments =
        partialPaymentConfig?.allowPartialPayments || false;
      const isFixedAmount =
    ...
    return (
      <div>
        ...
        {/* Only show error if amount is not fixed */}
        {error && !isFixedAmount && (
