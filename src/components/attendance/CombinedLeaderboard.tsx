...
export const CombinedLeaderboard: React.FC<CombinedLeaderboardProps> = ({
  ...
  const filteredCohorts = availableCohorts.filter(cohort =>
  ...
  const handleSelectAll = () => {
    ...
    const allFilteredSelected = filteredCohortIds.every(id => selectedCohorts.includes(id));
    
    if (allFilteredSelected) {
      // Deselect all filtered cohorts
      setSelectedCohorts(prev => prev.filter(id => !filteredCohortIds.includes(id)));
    } else {
  ...
  const allFilteredSelected = filteredCohorts.length > 0 && 
  ...
  const generatePublicLink = () => {
    ...
    const publicUrl = `${window.location.origin}/public/combined-leaderboard/${cohortIds}`;
