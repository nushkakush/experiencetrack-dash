...
export function CohortHolidayManagementDialog({
  ...
  const [publishedCohortHolidays, setPublishedCohortHolidays] = useState<Holiday[]>([]);
  const [publishedGlobalHolidays, setPublishedGlobalHolidays] = useState<Holiday[]>([]);
  ...
  const loadHolidays = async () => {
    setLoading(true);
    try {
      const [draftCohort, publishedCohort, publishedGlobal] = await Promise.all([
      ...
      const draftSelected: SelectedHoliday[] = draftCohort.map(holiday => ({
        id: holiday.id,
        date: holiday.date,
        title: holiday.title,
      ...
      setPublishedCohortHolidays(publishedCohort);
      setPublishedGlobalHolidays(publishedGlobal);

      // Set selected dates for calendar
      const draftDates = draftCohort.map(h => parseISO(h.date));
  ...
  const handleDateSelect = (date: Date | undefined) => {
    ...
    // Check if date is already a global holiday
    const isGlobalHoliday = publishedGlobalHolidays.some(h => h.date === dateString);
    if (isGlobalHoliday) {
      toast.error('This date is already a global holiday');
      return;
    }

    // Check if date is already a published cohort holiday
    const isPublishedCohortHoliday = publishedCohortHolidays.some(h => h.date === dateString);
    if (isPublishedCohortHoliday) {
      toast.error('This date is already a published cohort holiday');
      return;
    }
    ...
    try {
      ...
      if (newHolidays.length > 0) {
        const newHolidayRequests = newHolidays.map(holiday => ({
          ...
          holiday_type: 'cohort_specific' as const,
          cohort_id: cohortId,
  ...
  const publishHolidays = async () => {
    ...
    try {
      ...
      const currentDrafts = await HolidaysService.getCohortHolidays(cohortId, 'draft');
  ...
  // Get all holiday dates for calendar display
  const allHolidayDates = [
    ...publishedGlobalHolidays.map(h => parseISO(h.date)),
    ...publishedCohortHolidays.map(h => parseISO(h.date))
  ];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Manage cohort-specific holidays for {cohortName}. These holidays are in addition to global holidays.
          </DialogDescription>
        </DialogHeader>
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'draft' | 'live')}>
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="live" className="flex items-center gap-2">
              ...
              Live ({publishedCohortHolidays.length + publishedGlobalHolidays.length})
            </TabsTrigger>
          </TabsList>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <CalendarComponent
                  ...
                  modifiers={{
                    published: allHolidayDates,
                    global: publishedGlobalHolidays.map(h => parseISO(h.date)),
                    cohort: publishedCohortHolidays.map(h => parseISO(h.date))
                  }}
                  modifiersStyles={{
                    global: {
                      backgroundColor: 'hsl(262 83% 58%)', // A nice purple/violet
                      color: 'hsl(0 0% 100%)', // White text for contrast
                      fontWeight: 'bold',
                      border: '2px solid hsl(262 83% 48%)', // Slightly darker border
                      borderRadius: '4px'
                    },
                    cohort: {
                  ...
                  disabled={allHolidayDates}
                  className="rounded-md border"
                />
                <div className="flex flex-col gap-2 text-xs text-muted-foreground">
                  <div className="flex items-center gap-1">
                    <div className="w-3 h-3 bg-primary rounded"></div>
                    <span>Draft cohort holidays</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <div className="w-3 h-3 bg-destructive rounded"></div>
                    <span>Published cohort holidays</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <div className="w-3 h-3 rounded" style={{ backgroundColor: 'hsl(262 83% 58%)', border: '1px solid hsl(262 83% 48%)' }}></div>
          ...
          <TabsContent value="live" className="space-y-4">
            <div className="text-sm text-muted-foreground">
              Live holidays are currently active and affecting attendance calculations for {cohortName}.
            </div>

            {/* Cohort-specific holidays */}
            {publishedCohortHolidays.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <h3 className="text-lg font-semibold">Cohort Holidays</h3>
                  <Badge variant="secondary">{cohortName}</Badge>
                </div>
                {publishedCohortHolidays
                  .sort((a, b) => a.date.localeCompare(b.date))
                  .map((holiday) => (
                    <Card key={holiday.id}>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              ...
                              <Badge variant="destructive">Cohort</Badge>
                            </div>
            ...
            {/* Separator */}
            {publishedCohortHolidays.length > 0 && publishedGlobalHolidays.length > 0 && (
              <Separator className="my-6" />
            )}

            {/* Global holidays */}
            {publishedGlobalHolidays.length > 0 && (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <h3 className="text-lg font-semibold">Global Holidays</h3>
                  <Badge variant="outline">All Cohorts</Badge>
                </div>
                {publishedGlobalHolidays
                  .sort((a, b) => a.date.localeCompare(b.date))
                  .map((holiday) => (
                    <Card key={holiday.id} className="opacity-75">
                      <CardContent className="pt-4">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <h4 className="font-medium">{holiday.title}</h4>
