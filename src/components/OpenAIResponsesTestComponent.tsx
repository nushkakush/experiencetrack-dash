...
export function OpenAIResponsesTestComponent() {
  ...
  const handleWebSearchQuery = async () => {
    try {
      await searchAndAnalyze(searchQuery, 'Provide a comprehensive analysis with current information.');
    } catch (error) {
      ...
      {state.response && (
        <CardContent>
          <div className="space-y-4">
            ...
            {state.response.data?.toolsUsed && state.response.data.toolsUsed.length > 0 && (
              <div>
                <h4 className="font-semibold">Tools Used:</h4>
                <div className="flex flex-wrap gap-2">
                  {state.response.data.toolsUsed.map((tool: any, index: number) => (
                    <Badge key={index} variant={tool.success ? "default" : "destructive"}>
              ...
              <div>
                ...
                <div className="mt-2">
                  ...
                  <ul className="text-sm text-muted-foreground list-disc list-inside">
                    {state.response.data.reasoningSummary.keySteps.map((step: string, index: number) => (
    ...
    <div className="container mx-auto p-6 max-w-6xl">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          Test the new OpenAI Responses API with built-in tools and advanced features.
        </p>
      </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button onClick={handleBasicGeneration} disabled={basicState.isLoading || !prompt}>
                  {basicState.isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                  Generate Text
                </Button>
                <Button onClick={handleReasoning} disabled={basicState.isLoading || !prompt} variant="outline">
                  <Brain className="mr-2 h-4 w-4" />
                  With Reasoning
                </Button>
                <Button onClick={handleBackground} disabled={basicState.isLoading || !prompt} variant="outline">
        ...
        <TabsContent value="tools" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Test the built-in tools: Web Search, File Search, and Computer Use
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                <Button onClick={handleWebSearch} disabled={basicState.isLoading || !prompt}>
                  <Search className="mr-2 h-4 w-4" />
                  Web Search
                </Button>
                <Button onClick={handleFileSearch} disabled={basicState.isLoading || !prompt} variant="outline">
                  <FileText className="mr-2 h-4 w-4" />
                  File Search
                </Button>
                <Button onClick={handleComputerUse} disabled={basicState.isLoading || !prompt} variant="outline">
        ...
        <TabsContent value="web-search" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Test specialized web search functionality with research capabilities
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                <Button onClick={handleWebSearchQuery} disabled={webSearchState.isLoading || !searchQuery}>
                  <Search className="mr-2 h-4 w-4" />
                  Search & Analyze
                </Button>
                <Button onClick={handleCurrentInfo} disabled={webSearchState.isLoading || !searchQuery} variant="outline">
                  Get Current Info
                </Button>
                <Button onClick={handleResearchWithReasoning} disabled={webSearchState.isLoading || !searchQuery} variant="outline">
                  <Brain className="mr-2 h-4 w-4" />
                  Research + Reasoning
                </Button>
              </div>
      ...
      <div className="mt-8 p-4 bg-muted rounded-lg">
        ...
        <ul className="text-sm text-muted-foreground space-y-1">
          <li>• <strong>Web Search:</strong> Access to real-time web information</li>
          <li>• <strong>File Search:</strong> Search and analyze documents</li>
          <li>• <strong>Computer Use:</strong> Interact with applications and perform tasks</li>
          <li>• <strong>Reasoning Summary:</strong> Get insights into AI's thinking process</li>
          <li>• <strong>Background Mode:</strong> Handle long-running tasks asynchronously</li>
          <li>• <strong>Stateful Interactions:</strong> Maintain conversation context</li>
        </ul>
      </div>
    </div>
  );
