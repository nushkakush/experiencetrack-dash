...
import { Loader2, AlertCircle, Expand, CheckCircle, XCircle } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogDescription,
  ...
  const handleExpandAll = async () => {
    ...
    try {
      ...
      for (let i = 0; i < briefsToExpand.length; i++) {
        ...
        setExpansionProgress(prev => prev ? {
          ...prev,
          current: i,
      ...
      <DialogContent className='sm:max-w-lg max-h-[80vh] overflow-y-auto'>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            Each brief will be processed sequentially to ensure quality and avoid system overload.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className='space-y-4'>
            ...
            <div className='space-y-2'>
              <div className='flex items-center space-x-2'>
                <Checkbox 
                  ...
                  onCheckedChange={(checked) => setIncludeExpanded(checked as boolean)}
                ...
                >
                  Also re-expand the {expandedBriefs.length} already expanded brief{expandedBriefs.length !== 1 ? 's' : ''}
                </Label>
              </div>
            </div>
          )}
          ...
          <Alert>
            ...
            <AlertDescription>
              <strong>Estimated Cost:</strong> ~{totalToExpand} expansion credit{totalToExpand !== 1 ? 's' : ''} 
              (${(totalToExpand * 0.02).toFixed(2)}) • 
              <strong>Time:</strong> ~{Math.ceil(totalToExpand * 1.5)} minutes
            </AlertDescription>
          </Alert>
          ...
          {expansionProgress && (
            <div className='space-y-4'>
              ...
              {/* Status Summary */}
              {(expansionProgress.completed.length > 0 || expansionProgress.failed.length > 0) && (
                    ...
                    <div className='space-y-2'>
                      ...
                      <div className='max-h-20 overflow-y-auto space-y-1 border border-green-200 rounded bg-green-50 p-2'>
                        {expansionProgress.completed.map((title, index) => (
                          <div key={index} className='flex items-start gap-2 text-xs text-green-600'>
