...
import { Building2, Calendar, Expand, X, ExternalLink, Globe, RefreshCw, RotateCcw } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export const MagicBriefPreview: React.FC<MagicBriefPreviewProps> = ({
  ...
  const fetchRawResponseFromLogs = async () => {
    ...
    try {
      const rawResponse = await MagicBriefsService.getRawResponseFromLogs(brief.id);
  ...
  const handleRegenerate = async () => {
    try {
      ...
      toast.success('Magic brief regenerated successfully!', { id: 'regenerate-brief' });
      ...
      toast.error(error.message || 'Failed to regenerate magic brief', { id: 'regenerate-brief' });
    }
  };
              ...
              <div className="space-y-1">
                {brief.connected_learning_outcomes.map((outcome, index) => (
                  <div key={index} className="text-sm text-muted-foreground flex items-start gap-2">
                    ...
                    <span className="break-words overflow-wrap-anywhere">{outcome}</span>
                  </div>
                ))}
              </div>
            </div>
