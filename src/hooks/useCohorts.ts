  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      if (profile?.role === 'program_manager' || profile?.role === 'fee_collector') {
        ...
        const assignmentResponse = await CohortAssignmentService.getAssignedCohortsForUser(profile.user_id);
        if (!assignmentResponse.success) {
          Logger.getInstance().error('useCohorts: Failed to get assigned cohorts', { error: assignmentResponse.error });
          throw new Error(assignmentResponse.error?.message || "Failed to fetch assigned cohorts");
        }
        ...
        if (!allCohortsResponse.success) {
          Logger.getInstance().error('useCohorts: Service failed', { error: allCohortsResponse.error });
          throw new Error(allCohortsResponse.error || "Failed to fetch cohorts");
        }

        const allCohorts = allCohortsResponse.data || [];
        const assignedCohortIds = new Set(assignedCohorts.map(cohort => cohort.id));
