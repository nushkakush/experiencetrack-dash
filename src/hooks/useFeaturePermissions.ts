...
interface UseFeaturePermissionsReturn {
  ...
  canAccessFeeCollection: boolean;
  canAccessAttendance: boolean;
  canAccessCohortDetails: boolean;

  // Utility functions
  getRolePermissions: () => FeatureKey[];
  getFeatureMetadata: (
    feature: FeatureKey
  ) => (typeof FEATURE_METADATA)[FeatureKey] | null;
...
export const useFeaturePermissions = (): UseFeaturePermissionsReturn => {
  ...
  const checkPermissions = (features: FeatureKey[]): PermissionCheck[] => {
    return features.map(feature => ({
      ...
      feature,
      role: userRole,
      reason: permissionSet.has(feature)
        ? undefined
        : `User with role '${userRole}' does not have permission for '${feature}'`,
    }));
  };
  ...
  const canAccessAttendance = hasAnyPermission([
    'attendance.mark',
    'attendance.edit',
    'attendance.delete',
    'attendance.export',
  ...
  const canAccessCohortDetails = hasAnyPermission([
    'cohorts.edit',
    'cohorts.delete',
  ]);
