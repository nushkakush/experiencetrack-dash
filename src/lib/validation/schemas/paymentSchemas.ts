...
export const paymentPlanSchema = z.object({
  paymentPlan: z.enum(['one_shot', 'sem_wise', 'instalment_wise', 'not_selected'] as const, {
...
export const paymentSubmissionSchema = z.object({
  paymentMethod: z.enum(['cash', 'bank_transfer', 'cheque', 'razorpay', 'scan_to_pay'], {
  ...
  notes: z.string()
    ...
    .optional(),
  receiptFile: z.instanceof(File, { message: 'Please upload a valid receipt file' })
    .refine(
    ...
    .optional(),
  proofOfPaymentFile: z.instanceof(File, { message: 'Please upload a valid proof of payment file' })
    .refine(
      (file) => file.size <= 5 * 1024 * 1024,
      'File size must be less than 5MB'
    )
    ...
    .optional(),
  transactionScreenshotFile: z.instanceof(File, { message: 'Please upload a valid screenshot file' })
    .refine(
      (file) => file.size <= 5 * 1024 * 1024,
      'File size must be less than 5MB'
    )
...
export const scholarshipAssignmentSchema = z.object({
  ...
  additionalDiscountPercentage: z.number()
...
export const feeStructureSchema = z.object({
  ...
  instalmentsPerSemester: z.number()
  ...
  oneShotDiscountPercentage: z.number()
...
export const paymentPlanValidationSchema = z.object({
...
export type ScholarshipAssignmentInput = z.infer<typeof scholarshipAssignmentSchema>;
export type FeeStructureInput = z.infer<typeof feeStructureSchema>;
export type PaymentPlanValidationInput = z.infer<typeof paymentPlanValidationSchema>;
