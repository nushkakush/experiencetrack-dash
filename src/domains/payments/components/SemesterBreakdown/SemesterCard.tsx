...
export const SemesterCard: React.FC<SemesterCardProps> = React.memo(({
  ...
  const completionPercentage = semester.totalFee > 0 
  ...
  return (
    <Card className={semester.status === 'current' ? 'ring-2 ring-primary' : ''}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            ...
            <div className="font-semibold">{formatCurrency(semester.totalFee)}</div>
          </div>
          <div>
            <div className="text-muted-foreground">Paid</div>
            <div className="font-semibold text-green-600">{formatCurrency(semester.paidAmount)}</div>
          </div>
          <div>
            <div className="text-muted-foreground">Remaining</div>
            <div className="font-semibold text-orange-600">{formatCurrency(semester.remainingAmount)}</div>
          </div>
          <div>
            <div className="text-muted-foreground">Installments</div>
            <div className="font-semibold">{semester.installments.length}</div>
          </div>
        </div>
        ...
        {overdueInstallments.length > 0 && (
          <div className="flex items-center space-x-2 text-red-600 bg-red-50 p-2 rounded text-sm">
            ...
            <span>{overdueInstallments.length} overdue installment{overdueInstallments.length > 1 ? 's' : ''}</span>
          </div>
        )}
        ...
        {nextInstallment && (
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between p-2 bg-muted rounded">
              <div>
                <div className="text-sm font-medium">{formatCurrency(nextInstallment.amount)}</div>
                <div className="text-xs text-muted-foreground">
                  Due: {formatDate(nextInstallment.dueDate)}
                </div>
              </div>
              {onPayInstallment && (
                <Button size="sm" onClick={() => onPayInstallment(nextInstallment.id)}>
        ...
        <div className="flex space-x-2 pt-2">
          {onViewDetails && (
            <Button variant="outline" size="sm" onClick={() => onViewDetails(semester.id)} className="flex-1">
