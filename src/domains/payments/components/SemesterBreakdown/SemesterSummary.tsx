...
export const SemesterSummary: React.FC<SemesterSummaryProps> = React.memo(({ summary }) => {
  const progressColor = summary.completionPercentage >= 80 ? 'bg-green-500' : 
                       summary.completionPercentage >= 60 ? 'bg-yellow-500' : 'bg-red-500';
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Total Program Fee</CardTitle>
          <DollarSign className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{formatCurrency(summary.totalFee)}</div>
          <div className="flex items-center space-x-2 mt-2">
            <Progress value={summary.completionPercentage} className="flex-1 h-2" />
            <span className="text-sm text-muted-foreground">{summary.completionPercentage}%</span>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold text-orange-600">{formatCurrency(summary.remainingAmount)}</div>
          <p className="text-xs text-muted-foreground mt-2">
