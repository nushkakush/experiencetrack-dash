...
import { Calendar, IndianRupee, FileText, Download } from 'lucide-react';
...
export const SemesterBreakdownTable: React.FC<SemesterBreakdownTableProps> =
  React.memo(
    ({
      studentId,
      semesters,
      ...
      const filteredSemesters = React.useMemo(() => {
        return semesters.filter(semester => {
          if (filters.status !== 'all' && semester.status !== filters.status)
            return false;
          if (filters.semester !== 'all' && semester.id !== filters.semester)
            return false;
          return true;
        });
      }, [semesters, filters]);
      ...
      const summary = React.useMemo(() => {
        ...
        return {
          ...
          completedSemesters: semesters.filter(s => s.status === 'completed')
            .length,
