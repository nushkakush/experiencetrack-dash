...
export const PaymentPlanDisplay: React.FC<PaymentPlanDisplayProps> = React.memo(
  ({ paymentPlan, onEdit, onCustomize, loading = false }) => {
    const planOption = PAYMENT_PLAN_OPTIONS.find(
      option => option.value === paymentPlan.payment_plan
    );
        ...
        <CardHeader className='pb-3'>
          <div className='flex items-center justify-between'>
            <CardTitle className='flex items-center gap-2 text-lg'>
              ...
              {paymentPlan.is_custom ? 'Custom ' : ''}
              {planOption?.label || 'Payment Plan'}
            </CardTitle>
