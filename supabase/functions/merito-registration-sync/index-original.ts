...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const mapApplicationStatusToMeritto = (status: string): string => {
    ...
    const determineConversionStage = (): string => {
      ...
      const conversionStageMap: { [key: string]: string } = {
    ...
    // Map application status to Meritto-friendly status name
    const merittoStatus = mapApplicationStatusToMeritto(applicationStatus);
    addFieldIfExists(leadData, 'application_status', merittoStatus);
