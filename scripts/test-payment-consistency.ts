...
function calculatePaymentSchedule(
  ...
  if (paymentPlan === 'one_shot') {
    const discountPercentage = Number(feeStructure.one_shot_discount_percentage || 0);
...
async function testPaymentConsistency() {
  try {
    ...
    console.log(`   Program Fee: ₹${feeStructure.total_program_fee.toLocaleString()}`);
    console.log(`   Admission Fee: ₹${feeStructure.admission_fee.toLocaleString()}`);
    console.log(`   Semesters: ${feeStructure.number_of_semesters}`);
    console.log(`   Installments per Semester: ${feeStructure.instalments_per_semester}`);
    console.log(`   One-shot Discount: ${feeStructure.one_shot_discount_percentage}%\n`);
    ...
    for (const studentPayment of studentPayments) {
      ...
      console.log(`🧪 Testing ${student.first_name} ${student.last_name} (${student.email})`);
      console.log(`   Payment Plan: ${studentPayment.payment_plan}`);
      console.log(`   Scholarship: ${scholarship ? `${scholarship.amount_percentage}%` : 'None'}`);
      
      // Calculate expected payment schedule
      const scholarshipPercentage = scholarship ? Number(scholarship.amount_percentage) : 0;
      ...
      if (expectedInstallments === actualInstallments) {
        console.log(`   ✅ Installments: ${expectedInstallments} (expected) = ${actualInstallments} (actual)`);
      } else {
        console.log(`   ❌ Installments: ${expectedInstallments} (expected) ≠ ${actualInstallments} (actual)`);
        allTestsPassed = false;
      }
      ...
      if (studentPayment.payment_plan === 'instalment_wise') {
        const expectedSemesters = new Set(expectedSchedule.installments.map(i => i.semester_number)).size;
        const actualSemesters = new Set(actualSchedule.installments.map((i: any) => i.semester_number)).size;
        
        if (expectedSemesters === actualSemesters) {
          console.log(`   ✅ Semesters: ${expectedSemesters} (expected) = ${actualSemesters} (actual)`);
        } else {
          console.log(`   ❌ Semesters: ${expectedSemesters} (expected) ≠ ${actualSemesters} (actual)`);
          allTestsPassed = false;
        }
      ...
      if (Math.abs(expectedFirstAmount - actualFirstAmount) < 1) {
        console.log(`   ✅ First Installment: ₹${expectedFirstAmount.toLocaleString()} (expected) = ₹${actualFirstAmount.toLocaleString()} (actual)`);
      } else {
        console.log(`   ❌ First Installment: ₹${expectedFirstAmount.toLocaleString()} (expected) ≠ ₹${actualFirstAmount.toLocaleString()} (actual)`);
        allTestsPassed = false;
      }
    ...
    if (allTestsPassed) {
      ...
      console.log('The payment calculation service generates consistent results with the database.');
    } else {
      console.log('❌ Some payment consistency tests failed!');
      console.log('There are discrepancies between expected and actual payment calculations.');
    }
