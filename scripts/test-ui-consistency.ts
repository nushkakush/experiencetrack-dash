...
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdobXBhZ2h5YXN5bGxmdmFtZm5hIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTQ2NTI0NDgsImV4cCI6MjA3MDIyODQ0OH0.qhWHU-KkdpvfOTG-ROxf1BMTUlah2xDYJean69hhyH4";
...
function processPaymentSchedule(studentPayments: any[]) {
  ...
  const semesters = Array.from(semesterMap.entries()).map(([semesterNumber, installments]) => {
    const semesterTotal = installments.reduce((sum: number, inst: any) => sum + Number(inst.amount), 0);
    const semesterPaid = installments.reduce((sum: number, inst: any) => sum + Number(inst.amount_paid || 0), 0);
    const semesterPending = installments.reduce((sum: number, inst: any) => sum + Number(inst.amount_pending || 0), 0);
    ...
    return {
      ...
      instalments: installments.map((installment: any) => ({
...
async function testUIConsistency() {
  try {
    ...
    for (const studentPayment of studentPayments) {
      ...
      console.log(`🧪 Testing UI for ${student.first_name} ${student.last_name} (${student.email})`);
      ...
      const expectedSemesters = studentPayment.payment_plan === 'one_shot' ? 0 : 
                               studentPayment.payment_plan === 'sem_wise' ? 4 : 4;
      const actualSemesters = paymentBreakdown.semesters.length;
      
      if (expectedSemesters === actualSemesters) {
        console.log(`   ✅ Semesters Displayed: ${expectedSemesters} (expected) = ${actualSemesters} (actual)`);
      } else {
        console.log(`   ❌ Semesters Displayed: ${expectedSemesters} (expected) ≠ ${actualSemesters} (actual)`);
        allTestsPassed = false;
      }
      ...
      if (Math.abs(expectedTotal - actualTotal) < 1) {
        console.log(`   ✅ Total Amount: ₹${expectedTotal.toLocaleString()} (expected) = ₹${actualTotal.toLocaleString()} (actual)`);
      } else {
        console.log(`   ❌ Total Amount: ₹${expectedTotal.toLocaleString()} (expected) ≠ ₹${actualTotal.toLocaleString()} (actual)`);
        allTestsPassed = false;
      }
      ...
      if (studentPayment.payment_plan === 'instalment_wise') {
        const semesterGroups = paymentBreakdown.semesters.map(s => s.instalments.length);
        const expectedGroups = [3, 3, 3, 3]; // 3 installments per semester
        
        if (JSON.stringify(semesterGroups) === JSON.stringify(expectedGroups)) {
          console.log(`   ✅ Semester Groups: ${JSON.stringify(semesterGroups)} (expected) = ${JSON.stringify(expectedGroups)} (actual)`);
        } else {
          console.log(`   ❌ Semester Groups: ${JSON.stringify(semesterGroups)} (expected) ≠ ${JSON.stringify(expectedGroups)} (actual)`);
          allTestsPassed = false;
        }
        ...
        const semesterTotals = paymentBreakdown.semesters.map(s => s.totalPayable);
        const expectedSemesterTotal = studentPayment.payment_plan === 'instalment_wise' ? 
          (Number(studentPayment.payment_schedule.program_fee) / 4) : 0;
        
        const allSemesterTotalsCorrect = semesterTotals.every(total => 
          Math.abs(total - expectedSemesterTotal) < 1
        );
        
        if (allSemesterTotalsCorrect) {
          console.log(`   ✅ Semester Totals: All semesters have ₹${expectedSemesterTotal.toLocaleString()}`);
        } else {
          console.log(`   ❌ Semester Totals: Inconsistent amounts across semesters`);
          allTestsPassed = false;
        }
      }
    ...
    if (allTestsPassed) {
      ...
      console.log('The UI will correctly display all payment plans with proper semester grouping.');
    } else {
      console.log('❌ Some UI consistency tests failed!');
      console.log('There are issues with how the UI processes payment schedules.');
    }
